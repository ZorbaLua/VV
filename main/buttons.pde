...
void mousePressed() {
    ...
    else if (btn[1].MouseIsOver() && menuState == 1) { topPlayers = client.top3level(); }
    else if (btn[2].MouseIsOver() && menuState == 1) { client.logout(); menuState = 0; }
    else if (btn[3].MouseIsOver() )                  { /*menuState = 2;*/ }
    else if (btn[4].MouseIsOver() )                  { exit(); }
    else if (btn[5].MouseIsOver() && menuState == 0) {
        if(login()) { 
            menuState = 1; 
            playerInfo = client.info();
            topPlayers = client.top3level();
        }
    }
    else if (btn[6].MouseIsOver() && menuState == 0) { if(signin()) menuState = 0; }
}
